...
var _addListener = (element, type, func, nonPassive, capture) => element.addEventListener(type, func, { passive: !nonPassive, capture: !!capture });
...
var _getSizeFunc = (scroller, isViewport, { d, d2, a }) => (a = _getProxyProp(scroller, "getBoundingClientRect")) ? () => a()[d] : () => (isViewport ? _win2["inner" + d2] : scroller["client" + d2]) || 0;
var _getOffsetsFunc = (element, isViewport) => !isViewport || ~_proxies.indexOf(element) ? _getBoundsFunc(element) : () => _winOffsets;
var _maxScroll = (element, { s, d2, d, a }) => Math.max(0, (s = "scroll" + d2) && (a = _getProxyProp(element, s)) ? a() - _getBoundsFunc(element)()[d] : _isViewport2(element) ? (_docEl2[s] || _body2[s]) - (_win2["inner" + d2] || _docEl2["client" + d2] || _body2["client" + d2]) : element[s] - element["offset" + d2]);
  ...
  init(vars, animation) {
    ...
    self.scrubDuration = (value) => {
      ...
      if (!scrubSmooth) {
        ...
        scrubTween ? scrubTween.duration(value) : scrubTween = gsap2.to(animation, { ease: "expo", totalProgress: "+=0", duration: scrubSmooth, paused: true, onComplete: () => onScrubComplete && onScrubComplete(self) });
      }
    };
    if (animation) {
    ...
    self.enable = (reset, refresh) => {
      if (!self.enabled) {
        ...
        _addListener2(isViewport ? _doc2 : scroller, "scroll", _onScroll2);
  ...
  static enable() {
    ...
    if (gsap2) {
      ...
      if (_body2) {
        ...
        if (gsap2.matchMedia) {
          ScrollTrigger2.matchMedia = (vars) => {
...
var _getScrollNormalizer = (vars) => {
  ...
  tween = gsap2.to(self, { ease: "power4", paused: true, scrollX: normalizeScrollX ? "+=0.1" : "+=0", scrollY: "+=0.1", modifiers: { scrollY: _interruptionTracker(scrollFuncY, scrollFuncY(), () => tween.pause()) }, onUpdate: _updateAll, onComplete: onStopDelayedCall.vars.onComplete });
  return self;
};
